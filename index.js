...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++)
    {
        ...
        <p>Pledged:</strong> $${games[i].pledged.toLocaleString()}</p>
        <p>Goal:</strong> $${games[i].goal.toLocaleString()}</p>
        <p>Backers:</strong> ${games[i].backers.toLocaleString()}</p>
    `;
...
const descriptionText = `
    A total of $${totalRaised.toLocaleString()} has been raised for ${fundedGamesCount} ${fundedGamesCount === 1 ? "game" : "games"}. 
    Currently, ${unfundedGamesCount} ${unfundedGamesCount === 1 ? "game remains" : "games remain"} unfunded. 
    We need your help to get these projects funded!
`;
...
firstGameElement.innerHTML = `
    ${firstGame.name} with $${firstGame.pledged.toLocaleString()} pledged
`;
firstGameContainer.appendChild(firstGameElement);
// do the same for the runner up item
const secondGameElement = document.createElement("p");
secondGameElement.innerHTML = `
    ${secondGame.name} with $${secondGame.pledged.toLocaleString()} pledged
`;
...
// Grab the search bar input and button
...
function searchGames() {
    // Get the value of the search bar (convert to lowercase for case-insensitive matching)
    ...
    // Clear the current games container
    deleteChildElements(gamesContainer);
